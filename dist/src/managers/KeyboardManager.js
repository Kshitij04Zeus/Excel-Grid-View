...
export class KeyboardManager {
    ...
    canvas;
    render;
    constructor(selection, commandManager, datastore, viewport, canvas, render) {
    ...
    handleKeyDown(event) {
        ...
        switch (event.key) {
            ...
            case "ArrowDown":
                if (event.shiftKey)
                    endRow = Math.min(Constants.TOTAL_ROWS - 1, endRow + 1);
                else {
            ...
            case "ArrowRight":
                if (event.shiftKey)
                    endCol = Math.min(Constants.TOTAL_COLUMNS - 1, endCol + 1);
                else {
