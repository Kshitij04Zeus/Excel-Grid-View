...
export class FormulaParser {
    ...
    indexToCellRef(rowInd, colInd) {
        const colLetter = this.columnIndexToLetter(colInd);
        const rowNumber = rowInd + 1;
    ...
    evaluate(value) {
        ...
        expression = expression.replace(/SUM\(([A-Z0-9:]+)\)/g, (whole, rangeStr) => {
            const refs = this.expandRange(rangeStr);
            // ignore non-numeric values in the range
            const total = refs.reduce((sum, ref) => sum + (this.getCellNumericValue(ref) ?? 0), 0);
